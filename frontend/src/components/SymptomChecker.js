...
// List of symptoms (machine format)
const symptomsList = [
...
// Humanize function for display
const humanizeSymptom = (symptom) => {
  return symptom
...
const SymptomChecker = () => {
  ...
  const filteredSymptoms = symptomsList.filter(symptom =>
    humanizeSymptom(symptom).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSymptoms.length < 5) {
      setError("❗ Please select at least 5 symptoms before submitting.");
      return;
    }
        ...
        <div className="selected-symptoms">
          {selectedSymptoms.map(symptom => (
            <div key={symptom} className="selected-symptom">
              {humanizeSymptom(symptom)}
              <button type="button" onClick={() => handleRemoveSymptom(symptom)}>✖</button>
            </div>
          ))}
        </div>
